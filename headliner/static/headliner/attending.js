...
function getList(searchTerm, byLocation) {
    ...
    xhr.open("GET", "/headliner/get-attending", true)
    xhr.send()
}
...
function makeEventElement(item) {
    ...
    let pictureElement = '';
    if (item.picture) {
        pictureElement = `<img src="${item.picture}" alt="Event Picture" class="event-picture rounded-5 p-2">`;
    }
    element.innerHTML = `
        <div class="post-container text-white">
            <div class="title">
                <a href="/event/${item.id}" class="btn btn-dark btn-lg mb-3 display-5 fw-bold text-white link-underline link-underline-opacity-0" id="id_event_link_${item.id}">${item.title}</a>
            </div>
